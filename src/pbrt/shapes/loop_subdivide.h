...
inline double beta(int valence) {
    if (valence == 3) {
        return 3.f / 16.f;
    } else {
...
inline double loopGamma(int valence) {
    return 1.f / (valence + 3.f / (8.f * beta(valence)));
}

static Point3f weightOneRing(SDVertex *vert, double beta) {
...
static Point3f weightBoundary(SDVertex *vert, double beta) {
    // Put _vert_ one-ring in _pRing_
    int valence = vert->valence();
    auto pRing = std::vector<Point3f>(valence);
...
class LoopSubdivide {
    LoopSubdivide(const std::vector<int> &_vertex_indices, const std::vector<Point3f> &_p_limit)
        : vertex_indices(_vertex_indices), p_limit(_p_limit) {}
  ...
  public:
    ...
    static LoopSubdivide build(int nLevels, const std::vector<int> &vertexIndices,
        ...
        for (size_t i = 0; i < v.size(); ++i) {
            pLimit[i] = v[i]->boundary ? weightBoundary(v[i], 1.f / 5.f)
        ...
        for (SDVertex *vertex : v) {
            ...
            if (!vertex->boundary) {
                ...
                if (valence == 2)
                    ...
                    T = (-1 * pRing[0] + 2 * pRing[1] + 2 * pRing[2] + -1 * pRing[3] +
                         -2 * vertex->p)
                            .to_vector3();
                else {
                    double theta = compute_pi() / float(valence - 1);
                    T = (std::sin(theta) * (pRing[0] + pRing[valence - 1])).to_vector3();
                    for (int k = 1; k < valence - 1; ++k) {
                        double wt = (2 * std::cos(theta) - 2) * std::sin((k)*theta);
...

        {
            ...
            std::vector<int> vertex_indices(3 * n_triangles);
            int *ptr_vertex_indices = vertex_indices.data();
            ...
            return LoopSubdivide(vertex_indices, pLimit);
        }
    }
};
